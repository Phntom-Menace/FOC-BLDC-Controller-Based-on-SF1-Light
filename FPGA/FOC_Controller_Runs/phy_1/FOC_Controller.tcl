...
set rc [catch {
  ...
  import_db {../syn_1/FOC_Controller_gate.db}
  place
  export_db {FOC_Controller_place.db}
} RESULT]
if {$rc} {
...
set rc [catch {
  route
  report_area -io_info -file FOC_Controller_phy.area
  export_db {FOC_Controller_pr.db}
} RESULT]
if {$rc} {
...
set rc [catch {
  export_bid FOC_Controller_inst.bid
  bitgen -bit "FOC_Controller.bit"
} RESULT]
if {$rc} {
  step_error bitgen
  return -code error $RESULT
